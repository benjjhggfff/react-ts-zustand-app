...
import { loginApi, registerApi, fetchUserInfoApi } from '../../api/user' // 你的接口文件
import type {
  LoginParams,
...
// 1. 登录异步Thunk（适配Supabase响应）
export const loginAsync = createAsyncThunk<
  ApiResponse<{ token: string; user: UserInfo }>, // 成功返回类型：ApiResponse
  LoginParams, // 入参类型：LoginParams
  { rejectValue: string } // 失败抛出类型：string
>('user/login', async (params: LoginParams, { rejectWithValue }) => {
  try {
    // 强制指定返回类型为 ApiResponse，确保类型匹配
    const response: ApiResponse<{ token: string; user: UserInfo }> = await loginApi(params)
...
// 2. 注册异步Thunk（适配Supabase响应）
export const registerAsync = createAsyncThunk<
  ApiResponse<{ token: string; user: UserInfo }>, // 成功返回类型：ApiResponse
  LoginParams, // 入参类型：LoginParams
  { rejectValue: string } // 失败抛出类型：string
>('user/register', async (params: LoginParams, { rejectWithValue }) => {
  try {
    // 强制指定返回类型为 ApiResponse，确保类型匹配
    const response: ApiResponse<{ token: string; user: UserInfo }> = await registerApi(params)
    console.log(response)
    // 业务校验：code≠200 视为失败，抛出错误
    if (response.code !== 200) {
    ...
    // 异常分支：仅抛出字符串错误，绝不返回裸字符串
    const errorMsg = err?.msg || err?.message || '注册请求失败'
    return rejectWithValue(errorMsg)
  }
})

// 3. 获取用户信息异步Thunk（配套实现）
