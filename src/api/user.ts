...
// 上传头像接口
export async function upLoadFile(file: UploadFileInput): Promise<ApiResponse<string>> {
  let fileName = file.name | generateFileName(file.path)
}
...
const getMenuByRole = (role: string) => {
  const baseMenu = [
    {
      ...
      title: '排课管理',
      icon: 'icon-schedule',
      children: [{ path: '/schedule/edit', title: '排课管理', key: 'scheduleEdit' }],
    },
    {
      path: '/resources',
      title: '资源管理',
    ...
    },
    {
      ...
      title: '用户管理',
      icon: 'icon-stat',
      children: [{ path: '/user/userInfo', title: '个人信息', key: 'userInfo' }],
    },
  ]
  ...
  if (role === 'admin') {
    // 管理员拥有所有权限
    privileges = baseMenu.flatMap(menu => menu.children?.map(child => child.key) || [])
  } else if (role === 'teacher') {
    // 教师只能访问部分资源
    filteredMenu = baseMenu.map(menu => {
      if (menu.path === '/resources') {
        const filteredChildren = menu.children.filter(child =>
          ['classroomUsage', 'course'].includes(child.key)
        )
    ...
    filteredMenu = baseMenu.map(menu => {
      if (menu.path === '/resources') {
        const filteredChildren = menu.children.filter(child =>
          ['classroomUsage'].includes(child.key)
        )
        privileges = [...privileges, ...filteredChildren.map(child => child.key)]
        return {
          ...menu,
